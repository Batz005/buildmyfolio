...
export default function AwardBadge({ award }: AwardBadgeProps) {
  const hasImage = Boolean((award as any).imageUrl)
      ...
      <div className="relative z-[1] flex items-start gap-3">
        {hasImage ? (
          <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl bg-[radial-gradient(circle_at_30%_30%,rgba(110,245,227,0.38),rgba(13,148,136,0.1))] shadow-[0_12px_28px_-22px_rgba(13,148,136,0.55)]">
            ...
            <img src={(award as any).imageUrl as string} alt={award.title} className="h-10 w-10 object-contain" />
          </div>
        ) : (
        ...
        <div className="flex-1 space-y-2">
          ...
          {(award.date || award.blurb) && (
            <div className="flex flex-wrap gap-2 text-xs text-[color:var(--muted-foreground)]">
              ...
              {award.blurb && <span className="leading-relaxed">{award.blurb}</span>}
            </div>
          )}
        </div>
      </div>
